...
// Apply a theme class the way next-themes does: exactly one of `light` / `dark`
// on <html>. Quant X derives its DARK tokens from :root and overrides to LIGHT
// under `html.light`, so a bare `classList.toggle("dark")` (standard MagicUI)
// would leave `html.light` winning and the View-Transition snapshot stuck on
// the old theme. Removing both then adding the resolved class is correct for
// this app AND for a plain Tailwind `darkMode:"class"` setup.
...
export const AnimatedThemeToggler = ({
  ...
  const toggleTheme = useCallback(() => {
    ...
    const applyTheme = () => {
      // Always swap the class synchronously so the View Transitions API
