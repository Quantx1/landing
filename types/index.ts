...
/** Dual-mode 2026-06-12 — GET /api/managed/overview (beginner Home).
 *  Deterministic + honest-null; every health deduction is itemised. */
export interface ManagedOverview {
  health: {
    score: number
    label: 'Healthy' | 'Watch' | 'At risk'
    components: { key: string; label: string; impact: number; detail?: string | null }[]
  }
  pnl: {
    capital: number | null
  ...
  risk: {
    level: string | null
    flags: { key: string; severity: 'high' | 'medium' | 'low'; message: string }[]
    ok: boolean
    day_pnl: number | null
  ...
  autopilot: {
    available: boolean
    /** 'paper' = virtual money (Free always; Pro/Elite opt-in), 'live' = real broker account. */
    mode: 'paper' | 'live'
    enabled: boolean
    paused: boolean
  ...
  drawdown: { current_pct: number; as_of?: string | null } | null
}
...
// ============================================================================
// ASSISTANT USAGE — the daily chat-credit meter (the Copilot cap window).
// The legacy finance-assistant chat types were removed with its brain
// (2026-07-11 chat unification).
// ============================================================================
